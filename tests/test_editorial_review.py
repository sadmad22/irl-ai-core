...
def draft():
    return {
        ...
        "sections":[{"section_index":1,"heading":"Coverage","purpose":"explain coverage","body":"Professional liability coverage can address certain claims arising from professional services.","claims":[{"claim_id":"claim_1","text":"Professional liability coverage can address certain claims arising from professional services.","evidence_refs":["e1"],"grounding_status":"grounded"}]}],
...
def test_review_accepts_reader_facing_grounded_body():
    result = build_editorial_review(article_draft=draft())
    assert result["outcome"] == "approved"
    assert "research evidence records" not in draft()["sections"][0]["body"]
