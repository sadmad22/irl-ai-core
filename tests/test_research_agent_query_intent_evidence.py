...
def test_research_agent_writes_query_intent_evidence_end_to_end(tmp_path, monkeypatch):
    ...
    _write_json(
        project_dir / "search-metrics.json",
        {"keyword": "expat health insurance", "volume": 1000},
    )
    _write_json(
            ...
            "results": [
                {
                    ...
                    "snippet": "Learn about coverage and requirements.",
                ...
                },
                {
                    ...
                    "snippet": "Compare plans and pricing.",
    ...
    analysis = json.loads(
        (project_dir / "query-intent-analysis.json").read_text(encoding="utf-8")
    )
    evidence = json.loads(
    ...
    assert metadata["status"] == "research_started"
...
def test_research_agent_uses_stable_fallback_report_id(tmp_path, monkeypatch):
    ...
    _write_json(
        project_dir / "metadata.json",
        {"id": "", "status": "draft", "project_name": project_name},
    )
