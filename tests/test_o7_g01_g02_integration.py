...
def _artifacts() -> dict:
    return {
        "article_draft": {**LINEAGE, "lifecycle_stage": "draft_ready", "content_type": "guide", "primary_keyword": "expat health insurance", "title": "Expat Health Insurance Guide", "slug": "expat-health-insurance-guide", "sections": [{"section_id": "section_0", "heading": "Overview", "body": "Evidence-backed article text.", "purpose": "Explain the topic", "evidence_refs": ["evidence_1"], "claims": [{"claim_id": "claim_1", "text": "Evidence-backed article text.", "evidence_refs": ["evidence_1"], "grounding_status": "grounded"}]}], "tables": []},
...
def test_o7_g01_g02_canonical_optimization_reaches_assembly():
    ...
    assert assembly["lineage"]["optimization_id"] == OPTIMIZATION["optimization_id"]
    assert assembly["artifacts"]["optimization"] == {"seo_title": OPTIMIZATION["seo_title"], "meta_description": OPTIMIZATION["meta_description"]}
...
def test_o7_g01_g02_publication_intent_is_immutable():
    ...
    assert run["publication"]["mode"] == INTENT["mode"]
    assert run["publication"]["publish"] is False
    assert run["publication"]["human_approval_required"] is True
