...
from agents.research.article_package_engine import build_article_package
...
OPTIMIZATION = {
    ...
    "lineage": {k: LINEAGE[k] for k in ("report_id", "decision_id", "strategy_id", "brief_id")},
}
...
def _early() -> dict:
    return {
        "research_report": {}, "content_brief": {},
        "article_draft": copy.deepcopy(_artifacts()["article_draft"]),
        "article_draft_quality": copy.deepcopy(_artifacts()["quality"]),
        "editorial_review": copy.deepcopy(_artifacts()["editorial_review"]),
        "media": copy.deepcopy(_artifacts()["media"]),
        "linking": copy.deepcopy(_artifacts()["linking"]),
        "final_optimization": copy.deepcopy(OPTIMIZATION),
        "claim_audit": copy.deepcopy(_artifacts()["claim_audit"]),
        "seo_validation": {},
        "publication": {"gate_status": "allowed"},
    }


def _canonical_orchestration(*, live: bool = False) -> dict:
    calls = []
    ...
    result = orchestrator.build_production_orchestration(
        ...
        result=_early() | _artifacts(),
        deliver=True,
        delivery_mode="live" if live else "dry_run",
        adapter=adapter if live else None,
    )
    result["_captured_boundaries"] = calls
    return result
...
def test_o7_g01_g02_canonical_optimization_and_no_seo_substitution():
    result = _canonical_orchestration()
...
def test_o7_g01_g02_first_four_lineage_and_optimization_id_survive_assembly():
    ...
    for key in ("report_id", "decision_id", "strategy_id", "brief_id"):
        assert lineage[key] == LINEAGE[key]
...
def test_o7_g01_g02_six_id_lineage_optimization_and_seo_survive_package():
    ...
    for key, value in LINEAGE.items():
        assert package["lineage"][key] == value
...
def test_o7_g01_g02_o7_checkpoint_and_orchestration_traceability():
    orchestration = _canonical_orchestration()
    ...
    assert run["production_checkpoints"] == {
        "assembly_id": orchestration["production"]["assembly"]["assembly_id"],
        "package_id": orchestration["production"]["package"]["package_id"],
        "delivery_id": orchestration["production"]["boundary"]["delivery_id"],
    }
...
def test_o7_g01_g02_controlled_draft_reaches_human_review_and_preserves_intent():
    orchestration = _canonical_orchestration(live=True)
    monkeypatch = pytest.MonkeyPatch()
    try:
        monkeypatch.setattr(controlled_production, "run_production_orchestrator", lambda *a, **k: orchestration)
    ...
    assert orchestration["production"]["wordpress"]["publish"] is False
    assert orchestration["production"]["wordpress"]["human_approval_required"] is True
...
def test_o7_g01_g02_lineage_conflict_fails_closed_deterministically():
    ...
    assert failures[0] == ("LINEAGE_MISMATCH", ("report_id",))


def test_o7_g01_g02_missing_checkpoint_fails_closed():
    run = controlled_production.create_controlled_production_run(
        project_name="expat-health-insurance",
...
def test_o7_g01_g02_orchestration_and_wordpress_failures_fail_closed():
    failed = _canonical_orchestration()
    failed["lifecycle_stage"] = "failed"
    failed["error"] = {"type": "PackageError", "message": "package failed"}
    monkeypatch = pytest.MonkeyPatch()
    try:
    ...
    wordpress_failed = _canonical_orchestration(live=True)
    wordpress_failed["production"]["wordpress"]["delivery_status"] = "failed"
    wordpress_failed["production"]["wordpress"]["publish"] = False
    monkeypatch = pytest.MonkeyPatch()
    try:
...
def test_o7_g01_g02_canonical_boundary_is_the_one_seen_by_adapter():
    result = _canonical_orchestration(live=True)
    assert len(result["_captured_boundaries"]) == 1
    assert result["_captured_boundaries"][0] is result["production"]["boundary"]
