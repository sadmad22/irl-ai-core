    ...
    for item in editorial_evidence:
        for part in _coverage_sentences(item):
            if part.lower() not in seen:
                sentences.append(part); seen.add(part.lower())
    return "\n\n".join(sentences)
...
def _page_editorial_evidence(*, section_index: int, source_evidence: list[dict[str, Any]] | None) -> list[dict[str, Any]]:
    ...
    for item in source_evidence or []:
        if not isinstance(item, dict) or int(item.get("section_index", 0) or 0) != section_index: continue
        evidence_id = str(item.get("evidence_id", "")).strip(); text = str(item.get("text", "")).strip()
        source = item.get("source") if isinstance(item.get("source"), dict) else {}; url = str(source.get("url", "")).strip()
        provenance = item.get("provenance") if isinstance(item.get("provenance"), dict) else {}
        if not evidence_id or not text or not url or evidence_id in seen or provenance.get("verification") != "page_reviewed": continue
        record = dict(item); record["status"] = "ready"; normalized.append(record); seen.add(evidence_id)
    return normalized
...
def _coverage_editorial_evidence(*, serp_results: list[dict[str, Any]] | None, source_evidence: list[dict[str, Any]] | None) -> list[dict[str, Any]]:
    ...
    seen: set[str] = set()
    for item in source_evidence or []:
        if not isinstance(item, dict) or int(item.get("section_index", 0) or 0) != 3: continue
        evidence_id = str(item.get("evidence_id", "")).strip(); text = str(item.get("text", "")).strip(); source = item.get("source") if isinstance(item.get("source"), dict) else {}; url = str(source.get("url", "")).strip()
        if not evidence_id or not text or not url or evidence_id in seen: continue
        record = dict(item); record.setdefault("status", "ready"); provenance = dict(record.get("provenance") or {}); provenance.setdefault("verification", "page_reviewed"); record["provenance"] = provenance; normalized.append(record); seen.add(evidence_id)
    for item in serp_results or []:
        if not isinstance(item, dict): continue
        text = str(item.get("snippet") or item.get("description") or item.get("text") or "").strip(); source = item.get("source") if isinstance(item.get("source"), dict) else {}; url = str(item.get("url") or source.get("url") or "").strip()
        if not text or not url: continue
        digest = hashlib.sha256(url.encode("utf-8")).hexdigest()[:16]; evidence_id = f"editorial_serp_{digest}"
        if evidence_id in seen: continue
        normalized.append({"evidence_id": evidence_id, "section_index": 3, "status": "candidate", "text": text, "evidence_refs": [evidence_id], "source": {"type": "web_page", "url": url, "title": str(item.get("title") or source.get("title") or "").strip()}, "provenance": {"artifact": "serp-analysis.json", "method": "serp-snippet-editorial-v1", "verification": "snippet_only"}, "domain": "editorial"}); seen.add(evidence_id)
    return normalized
...
def _section_body(*, heading: str, keyword: str, evidence_records: list[dict[str, Any]], editorial_evidence: list[dict[str, Any]] | None = None) -> str:
    ...
    if normalized_heading == "coverage and key factors": return _coverage_body(editorial_evidence or [])
    if normalized_heading == "what you need to know": return _page_editorial_body(editorial_evidence or [])
    if not evidence_records: return ""
    if normalized_heading == "introduction": return _introduction_body(keyword=keyword, evidence_records=evidence_records)
...
def build_article_draft(*, content_brief: dict[str, Any], evidence_records: list[dict[str, Any]] | None = None, serp_results: list[dict[str, Any]] | None = None, source_evidence: list[dict[str, Any]] | None = None) -> dict[str, Any]:
    brief_id = str(content_brief.get("brief_id", "")).strip(); report_id = str(content_brief.get("report_id", "")).strip(); decision_id = str(content_brief.get("decision_id", "")).strip(); strategy_id = str(content_brief.get("strategy_id", "")).strip()
    if not all((brief_id, report_id, decision_id, strategy_id)): raise ValueError("Content Brief lineage identifiers are required")
    if content_brief.get("lifecycle_stage") != "content_brief_ready": raise ValueError("Article Draft requires a content_brief_ready Content Brief")
    refs = content_brief.get("evidence_refs")
    if not isinstance(refs, list) or not refs: raise ValueError("Content Brief must contain explicit evidence_refs")
    outline = content_brief.get("outline")
    if not isinstance(outline, list) or not outline: raise ValueError("Content Brief must contain a non-empty outline")
    content_type = content_brief.get("content_type"); keyword = str(content_brief.get("primary_keyword", "")).strip()
    if content_type not in {"guide", "comparison", "buyer_guide", "article"}: raise ValueError("Content Brief.content_type is invalid")
    if not keyword: raise ValueError("Content Brief.primary_keyword is required")
    normalized_refs = list(dict.fromkeys(str(ref).strip() for ref in refs if str(ref).strip())); ref_set = set(normalized_refs)
    ...
    editorial_evidence: list[dict[str, Any]] = []; sections = []; section_evidence_contracts: list[dict[str, Any]] = []
    for index, (item, refs_for_section) in enumerate(zip(outline, section_refs), 1):
        heading = str(item["heading"]).strip(); section_records = [grounded_records[ref] for ref in refs_for_section if ref in grounded_records]
        if index == 3 and heading.lower() == "coverage and key factors": section_editorial = coverage_editorial_evidence
        elif index == 2 and heading.lower() == "what you need to know": section_editorial = section2_editorial_evidence
        else: section_editorial = []
        if section_editorial:
            editorial_evidence.extend(section_editorial); refs_for_section = [record["evidence_id"] for record in section_editorial]
    ...
    all_grounding_records = list(grounded_records.values()) + coverage_editorial_evidence + section2_editorial_evidence
    ...
    for index, (section, claims) in enumerate(zip(sections, claims_by_section), 1):
        if section["heading"].lower() == "coverage and key factors": section["claims"] = _coverage_claims(coverage_editorial_evidence)
        elif section["heading"].lower() == "what you need to know": section["claims"] = _page_editorial_claims(section_index=index, editorial_evidence=section2_editorial_evidence)
        else: section["claims"] = claims
    ...
    payload = {"title": _title(content_brief), "content_type": content_type, "primary_keyword": keyword, "sections": sections, "evidence_refs": top_level_refs, "editorial_evidence": editorial_evidence, "section_evidence_contracts": section_evidence_contracts, "editorial_constraints": list(dict.fromkeys(str(value) for value in content_brief.get("editorial_constraints", []) if str(value).strip()))}
