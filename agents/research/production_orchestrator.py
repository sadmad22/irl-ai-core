...
def _orchestration_id(project_name: str, completed_stages: list[str], lineage: dict[str, str]) -> str:
    raw = json.dumps(
        {"project_name": project_name, "completed_stages": completed_stages, "lineage": lineage, "schema_version": SCHEMA_VERSION},
        sort_keys=True,
        ensure_ascii=False,
    )
...
def _completed_stages(result: dict[str, Any]) -> list[str]:
    """Translate outputs of the established composite pipeline into checkpoints."""
    completed: list[str] = []
    ...
    if isinstance(result.get("editorial_review"), dict):
        completed.extend(["editorial_cleanup", "media", "linking"])
    if isinstance(result.get("seo_validation"), dict):
...
def build_production_orchestration(*, project_name: str, result: dict[str, Any]) -> dict[str, Any]:
    """Build an orchestration record from the existing production pipeline output."""
...
def run_production_orchestrator(
    ...
    *,
    ...
    """Coordinate the established Core pipeline and produce the Phase 2 package."""
...
def execute_stage_plan(
    *,
    ...
    This primitive is intentionally in-memory. Persistent Production Job State
    belongs to Phase 4.
    """
    ...
    lineage = _lineage_from_result(context)
    return {
        "orchestration_id": _orchestration_id(project_name, list(dict.fromkeys(completed)), lineage),
        "project_name": project_name,
        "schema_version": SCHEMA_VERSION,
