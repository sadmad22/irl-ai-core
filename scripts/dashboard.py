...
<section class="card" style="margin-top:18px"><div class="toolbar"><div><h3 style="margin:0">DataForSEO Account &amp; Cost Monitor</h3><div class="muted">Live balance, current spending, API pricing, and an operational article-cost estimate.</div></div><span class="badge" id="costState">Checking…</span></div><div class="monitor-grid" id="costMetrics"></div><div id="costError"></div><div class="scroll" id="pricingTable"></div></section>
...
function renderCost(a,delivered){const state=document.getElementById('costState'),metrics=document.getElementById('costMetrics'),error=document.getElementById('costError'),table=document.getElementById('pricingTable');if(!a.configured){state.textContent='Not configured';state.className='badge bad';metrics.innerHTML='';table.innerHTML='';error.innerHTML='<div class="muted">Set DATAFORSEO_LOGIN and DATAFORSEO_PASSWORD in the production environment, then refresh.</div>';return}if(a.error){state.textContent='Unavailable';state.className='badge bad';metrics.innerHTML='';table.innerHTML='';error.innerHTML=`<div class="muted">${esc(a.error)}</div>`;return}state.textContent='Connected';state.className='badge ok';const today=Number(a.today_spend||0);const articleEstimate=delivered>0?today/delivered:null;metrics.innerHTML=[['Balance',money(a.balance),'live account balance'],['Today spend',money(today),'DataForSEO current-day spend'],['Estimated / article',articleEstimate===null?'—':money(articleEstimate),'today spend ÷ delivered drafts'],['Pricing entries',(a.pricing||[]).length,'available basic API price records']].map(x=>`<div class="metric"><div class="muted">${x[2]}</div><strong>${x[1]}</strong><span class="muted">${x[0]}</span></div>`).join('');error.innerHTML='<div class="muted" style="margin:8px 0">Article cost is an operational estimate until per-job balance snapshots are enabled; DataForSEO pricing excludes additional paid parameters.</div>';const rows=(a.pricing||[]).slice(0,30);table.innerHTML=rows.length?`<table><tr><th>API</th><th>Function</th><th>Charge</th><th>Cost</th></tr>${rows.map(x=>`<tr><td>${esc(x.api)}</td><td>${esc(x.function)}</td><td>${esc(x.cost_type)}</td><td>${money(x.cost)}</td></tr>`).join('')}</table>`:'<div class="muted">No pricing records returned.</div>'}
async function load(){const r=await fetch('/api/dashboard');const d=await r.json();const m=d.metrics,p=d.provider;document.getElementById('cards').innerHTML=[['Projects',m.projects,'initialized'],['WordPress Drafts',m.drafts,'delivered + still draft'],['Draft Ready',m.draft_ready,'ready before delivery'],['Active Runs',m.active_jobs,'current session']].map(x=>`<div class="card"><div class="muted">${x[2]}</div><div class="num">${x[1]}</div><strong>${x[0]}</strong></div>`).join('');document.getElementById('provider').innerHTML=`<span class="badge ok">${esc(p.active)}</span> <strong>${p.dataforseo.configured?'DataForSEO connected':'DataForSEO not configured'}</strong><div class="muted" style="margin-top:8px">Credentials are read from environment variables only and never displayed or stored by the UI.</div>`;renderCost(d.dataforseo,d.metrics.drafts);document.getElementById('projectsTable').innerHTML=d.recent_projects.length?`<table><tr><th>Project</th><th>Status</th><th>Draft</th></tr>${d.recent_projects.map(x=>`<tr><td><strong>${esc(x.project)}</strong><div class="muted">${esc(x.keyword)}</div></td><td>${esc(x.status||'created')}</td><td>${x.post_id?'#'+esc(x.post_id):'—'}</td></tr>`).join('')}</table>`:'<div class="muted">No projects yet.</div>';const drafts=d.recent_projects.filter(x=>x.post_id&&x.remote_status==='draft');document.getElementById('draftsTable').innerHTML=drafts.length?`<table><tr><th>Project</th><th>Post</th><th>Action</th></tr>${drafts.map(x=>`<tr><td>${esc(x.project)}</td><td>#${esc(x.post_id)}</td><td><a href="${esc(x.edit_url)}" target="_blank">Open Draft</a></td></tr>`).join('')}</table>`:'<div class="muted">No WordPress drafts recorded.</div>'}
...
class DashboardHandler(web_ui.Handler):
    server_version = "IRLAICoreDashboard/1.2"
    ...
    def do_GET(self) -> None:
        ...
        if path == "/api/dashboard":
            ...
            self._send(200, {"provider": web_ui.provider_status(), "dataforseo": dataforseo, "metrics": {"projects": len(projects), "drafts": len(delivered), "draft_ready": len(draft_ready), "active_jobs": sum(1 for job in jobs if job.get("status") in {"queued", "running"})}, "recent_projects": projects[:8]})
            return
        super().do_GET()
